...
public class RegularPanel extends JPanel {
	...
	private SelectionInList items;
	private PresentationModel beanModel;
	private RegularBean bean;
	...
	private void initComponents() {
		 ...
		 * Init the models 1st
		 */
		bean = new RegularBean("Regular Bean");
		beanModel = new PresentationModel(bean);
		items = new SelectionInList(beanModel.getModel("items"));
		bean.addItem("Test 1");
		/*
		...
		timeValue = 
			BasicComponentFactory.createLabel(beanModel.getModel("timeValue"));
		calculatedValue = 
			BasicComponentFactory.createLabel(beanModel.getModel("calculatedValue"));
		nameField = 
			BasicComponentFactory.createTextField(beanModel.getModel("name"));
		nameListenerLabel = 
			BasicComponentFactory.createLabel(beanModel.getModel("name"));
		selectedItem = 
			BasicComponentFactory.createTextField(items.getSelectionHolder());
		...
		this.setBorder(UIUtils.createTitledBorder("Non-AOP Version"));
