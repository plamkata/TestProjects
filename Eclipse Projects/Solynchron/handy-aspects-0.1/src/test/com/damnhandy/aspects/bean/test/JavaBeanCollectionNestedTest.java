...
public class JavaBeanCollectionNestedTest extends TestCase {
	...
	public void testParentPropertyChange() throws Exception {
		...
		assertEquals(listener.getChangeCount(), 1 + 4 * nestedList.size());
		...
		// 1 base + 4 old nested property changes + 2 * 4 new nested property changes
		assertEquals(listener.getChangeCount(), 1 + 4 * nestedList.size() + 4 * nestedList1.size());
		...
		// 1 base + 4 nested property changes
		assertEquals(listener.getChangeCount(), 1 + 4 * nestedSet.size());
	...
	public void testListNestedSpecificPropertyChange() throws Exception {
		...
		// nested property "nestedList1.name" should have changed only once
		assertTrue(listener.hasChangedOnce());
		listener.reset();
		...
		// nested property "nestedList1.name" should have changed once
		assertTrue(listener.hasChangedOnce());
		listener.reset();
	...
	public void testReorderedNestedPropertyChange() throws Exception {
		...
		assertEquals(listener.getChangeCount(), 2 * 5);
		assertEquals(listener.getIndex(), 1);
		//assertEquals(listener.getOldValue(), null);
