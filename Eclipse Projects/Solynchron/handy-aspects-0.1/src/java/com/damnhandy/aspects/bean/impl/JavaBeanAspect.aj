...
public aspect JavaBeanAspect extends AbstractJavaBeanAspect {
	...
	private void addNestedSupport(JavaBean bean, String fieldName,
		...
		if (oldValue == null && newValue == null) {
			...
			fireNestedChanges(bean, fieldName, oldValue, newValue);
		}
	}
	...
	private void fireNestedChanges(JavaBean bean, String fieldName, 
		...
		try {
			...
			PropertyDescriptor[] properties = value.beanInfo().getPropertyDescriptors();
			for (int i = 0; i < properties.length; i++) {
				PropertyDescriptor property = properties[i];
				String fieldName1 = property.getName();
				if (!property.isHidden() && !property.isConstrained() && !fieldName1.equals("class") && 
						!fieldName1.equals("beanInfo") && !fieldName1.equals("propertyChangeListeners")) {
					String nestedName = fieldName + "." + fieldName1;
					...
					bean.firePropertyChange(nestedName, oldNestedValue, newNestedValue);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
