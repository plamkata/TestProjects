...
public class ProblemLogic implements ILogic {
	...
	throws InitializationProgressException, ParseException {
		...
		if (searchString != null && !searchString.trim().equals("")) {
			...
			String[] fields = new String[]{"description", "type.name"};
			MultiFieldQueryParser parser = new MultiFieldQueryParser(
					fields, txtEM.getSearchFactory().getAnalyzer(Problem.class));
			org.apache.lucene.search.Query query = parser.parse(searchString);
			// wrap Lucene query in a javax.persistence.Query
			javax.persistence.Query q = txtEM.createFullTextQuery(query, Problem.class);
			q.setFirstResult(0);
			q.setMaxResults(20);
		...
		if (foundProblems == null) {
			...
			for (Problem p : problems) {
				// TODO: make sure that you initialize all problems
				foundProblems.add(p);
			}
		}
