...
public class ProblemFormMultipageEditor extends FormEditor {
	 ...
	 */
	public int openProblem(IEditorInput input) throws PartInitException {
		String id = "new";
		if (input instanceof FormEditorInput) {
			FormEditorInput formInput = (FormEditorInput) input;
			Problem problem = (Problem) formInput.getModels()[0];
			if (!problem.isNew()) {
				id = problem.getId().toString();
			}
		}
		ProblemForm form = new ProblemForm(
				Resources.getResource("ProblemForm.title") + " " + id, 
