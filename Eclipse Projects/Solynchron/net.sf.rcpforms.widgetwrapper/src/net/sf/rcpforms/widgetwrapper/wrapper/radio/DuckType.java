...
/**
 * DuckType. Implements Duck Typing for Java. ("If it walks like a duck, quacks like a duck,
 * it..."). Essentially allows programs to treat objects from separate hierarchies as if they were
 * designed with common interfaces as long as they adhere to common naming conventions.
 * <p>
 * This version is the strict DuckType. All methods present in interfaceToImplement must be present
...
public class DuckType implements InvocationHandler
{
    ...
    /**
     * Interface DuckType#Wrapper. An interface for DuckType proxies that allows clients to access
     * the proxied value. The value returned by calling DuckType#implement always implements this
     * interface.
     */
    ...
    {
        return Proxy.newProxyInstance(interfaceToImplement.getClassLoader(), new Class[]{
                interfaceToImplement, Wrapper.class}, new DuckType(object));
    }

    /**
     * Indicates if object is a (DuckType) instace of intrface. That is, is every method in intrface
     * present on object.
     * 
    ...
    {
        if (method.getName().equals("equals") && args != null && args.length == 1) //$NON-NLS-1$
        {
            return new Boolean(equals(args[0]));
        }
        if (method.getName().equals("hashCode") && args == null) //$NON-NLS-1$
        {
            return new Integer(hashCode());
        }
        if (method.getName().equals("duckType_GetWrappedValue") && args == null) //$NON-NLS-1$
        {
            return object;
        }
